...
public class v2{
      ...
      if (hasA(s, " ")) {
        while (i < s.length()) {
          if (s.substring(i,i+1).equals(" ")) {
            if (beginsWithCapital(s.substring(0, 1))) {
    ...
    public static String translator( String w ) {
        ...
        while (p > -1) {
           ...
           /* if (beginsWithCapital(word))
                word = capitalize(engToPigPunct(word));
            else*/
                word = engToPigPunct(word);
            s = s.substring(p + 1);
            result = result + " " + word;
        }

        if (beginsWithCapital(s))
            word = capitalize(engToPigPunct(s));
        else
            word = engToPigPunct(s);
        ...
        public static void main( String[] args ) {
                ...
                System.out.println( engToPigPunct(",.") );
          			System.out.println( translateSentence("Hi there, Mr. Mykolyk") );
